...
int64_t Instant::get(ChronoUnit u)const{
    ...
    throw "Illegal State: Wrong Enum";
}
...
Instant Instant::fromChronoTimePoint(const std::chrono::time_point& time){
	auto epochTime = time.time_since_epoch();
	auto secondtime = std::chrono::duration_cast<std::chrono::seconds>(epochTime);
	auto nanotime = std::chrono::duration_cast<std::chrono::nanoseconds>(epochTime-secondtime);
	int64_t seconds = secondtime.count();
	int nanos= nanotime.count()%NANOS_PER_SECOND;
...
Instant Instant::fromSFMLTime(const sf::Time& time){
	int64_t seconds = (int64_t) time.asSeconds();
	int nanos = (time.asMicroseconds()%1000000)*1000;
	return Instant(seconds,nanos);
}

std::chrono::time_point Instant::toTimePoint()const{
	std::chrono::seconds epochSeconds = seconds;
	std::chrono::nanoseconds nanos = nanos;
	auto dur = std::chrono::duration_cast<std::chrono::nanoseconds>(epochSeconds)+nanos;
	return std::chrono::time_point(dur);
}

sf::Time Instant::toSFMLTime()const{
	int64_t microseconds = get(ChronoUnit::MICROSECONDS);
	return sf::microseconds(microseconds);
}

Instant::Instant(const std::chrono::time_point& time){
	auto epochTime = time.time_since_epoch();
	auto secondtime = std::chrono::duration_cast<std::chrono::seconds>(epochTime);
	auto nanotime = std::chrono::duration_cast<std::chrono::nanoseconds>(epochTime-secondtime);
	seconds = secondtime.count();
	nanos= nanotime.count()%NANOS_PER_SECOND;
}

Instant::Instant(const sf::Time& time){
	seconds = (int64_t) time.asSeconds();
	nanos = (time.asMicroseconds()%1000000)*1000;
}


DataOutputStream& operator<<(DataOutputStream& strm,const Instant& i){
	strm<<i.toEpochSecond()
			<<i.getNanos();
	return strm;
}
DataOutputStream& operator<<(DataOutputStream& strm,const Duration& d){
	strm<<d.getSeconds()
			<<d.getNanos();
	return strm;
}

DataInputStream& operator>>(DataInputStream& strm,Instant& i){
	int64_t seconds(0);
	int nanos(0);
	strm >> seconds >> nanos;
	i = Instant::fromEpochSecond(seconds, nanos);
	return strm;
}

DataInputStream& operator>>(DataInputStream& strm,Duration& d){
	int64_t seconds(0);
	int nanos(0);
	strm >> seconds >> nanos;
	d = Duration::ofSeconds(seconds, nanos);
	return strm;
}
